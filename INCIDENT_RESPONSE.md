...
4. Rotate `MARKET_ADMIN_PASSWORD`, `ADMIN_JWT_SECRET`, `DATA_ENCRYPTION_KEY`, `SELLER_ADMIN_SECRET`, `IP_HASH_SECRET`, and `CAPTCHA_SECRET` in Render. After validating the new owner password, store its bcrypt cost-12 value in `MARKET_ADMIN_PASSWORD_HASH` and remove `MARKET_ADMIN_PASSWORD`.
...
7. Apply `supabase-security-2fa.sql`, then `supabase-auth-rate-limits.sql` in Supabase SQL Editor.
8. Force a clean Render deploy and confirm `/api/health` reports the expected current build.
9. Reset every shop panel password from the owner panel. Existing shop credentials and staff sessions are intentionally rejected by this build.
...
- Five failed privileged account attempts trigger a persistent account lock and repeated IP failures trigger an IP lock.
