...
7. Добавлено автоматическое зеркалирование финансовых данных в SQL-таблицы, если они созданы:
   - заказы -> `orders`;
   - депозиты -> `wallet_deposits`;
   - выводы -> `wallet_withdrawals`;
   - ledger -> `ledger_entries`;
   - NOWPayments IPN -> `payment_ipn_events`.

8. Подтверждено, что заблокированные пользователи уже отсекаются:
   - при обычном логине;
   - при Telegram-логине;
...
1. Нужно применить `supabase-schema.sql` в Supabase, чтобы включить подготовленное SQL-зеркало:
   - `orders`
   - `walletDeposits`
   - `walletWithdrawals`
   - `ownerLedger`
...
2. Нужны транзакции/locks для балансов и выводов.
   Сейчас добавлена защита от дублей и SQL-зеркало, но полная защита от параллельных гонок требует переключить расчеты денег с JSON-state на SQL-таблицы и атомарные операции.
